...
// TODO(@IvanMalison): Is there not a better way to get the actual import path of a package?
...
func (g *Generator) GeneratePrologue(pkg string) {
	g.populateImports()
	g.printf("// Package %s contains autogenerated mocks.\n", pkg)
	g.printf("package %s\n\n", pkg)
	g.generateImports()
	g.printf("\n")
}
...
func (g *Generator) GeneratePrologueComment(note string) {
	g.printf("// Code autogenerated by mockery v%s\n", Version)
	g.printf("//\n")
	g.printf("// Do not manually edit the content of this file.\n")
	g.printf("\n")
	if note != "" {
...
func (g *Generator) Write(w io.Writer) error {
	...
	if err != nil {
		line := "--------------------------------------------------------------------------------------------"
		fmt.Fprintf(os.Stderr, "Between the lines is the file (mock.go) libmockery generated in-memory but detected as invalid:\n%s\n%s\n%s\n", line, g.buf.String(), line)
		return err
	}
