...
func (s *GeneratorSuite) checkGeneration(filepath, interfaceName, expected string) *Generator {
	generator := s.getGenerator(filepath, interfaceName)
...
`
	s.checkGeneration(testFile, "Requester", expected)
}
...
`
	s.checkGeneration(testFile2, "Requester2", expected)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester3.go"), "Requester3",
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester4.go"), "Requester4",
		expected,
	)
}

func (s *GeneratorSuite) TestGeneratorPrologue() {
	generator := s.getGenerator(testFile, "Requester")
	expected := `// Package mocks contains autogenerated mocks.
package mocks

import "github.com/stretchr/testify/mock"
import test "github.com/shoenig/mockery3/v3/libmockery/fixtures"
...
func (s *GeneratorSuite) TestGeneratorPrologueWithImports() {
	generator := s.getGenerator("requester_ns.go", "RequesterNS")
	expected := `// Package mocks contains autogenerated mocks.
package mocks
...
func (s *GeneratorSuite) TestGeneratorPrologueWithMultipleImportsSameName() {
	generator := s.getGenerator("same_name_imports.go", "Example")

	expected := `// Package mocks contains autogenerated mocks.
package mocks

import fixtureshttp "github.com/shoenig3/mockery/v3/libmockery/fixtures/http"
import "net/http"
import "github.com/stretchr/testify/mock"
import test "github.com/shoenig3/mockery/v3/libmockery/fixtures"
...
func (s *GeneratorSuite) TestGeneratorPrologueNote() {
	generator := s.getGenerator(testFile, "Requester")
	generator.GeneratePrologueComment("A\\nB")

	expected := `// Code autogenerated by mockery v2.0.0
//
...
func (s *GeneratorSuite) TestVersionOnCorrectLine() {
	gen := s.getGenerator(testFile, "Requester")

	//Run everything that is ran by the GeneratorVisitor
	gen.GeneratePrologueComment("A\\nB")
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_iface.go"), "RequesterIface", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_ptr.go"), "RequesterPtr", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_slice.go"), "RequesterSlice", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_array.go"), "RequesterArray", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_ns.go"), "RequesterNS", expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsImport", expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsNamedImport", expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsPkg", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "custom_error.go"), "KeyManager", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_elided.go"), "RequesterElided", expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterReturnElided", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "func_type.go"), "Fooer", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "async.go"), "AsyncProducer", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "io_import.go"), "MyReader", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "consul.go"), "ConsulLock", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "empty_interface.go"), "Blank", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "map_func.go"), "MapFunc", expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "same_name_imports.go"), "Example", expected,
	)
}
...
`
	s.checkGeneration(
		"imports_same_as_package.go", "ImportsSameAsPackage", expected,
	)
}

//func (s *GeneratorSuite) TestPrologueWithImportSameAsLocalPackage() {
//	generator := s.getGenerator(
//		"imports_same_as_package.go", "ImportsSameAsPackage",
//	)
...
//import "github.com/stretchr/testify/mock"
//import "github.com/shoenig/mockery3/v3/libmockery/fixtures/test"
//
//`
//
//	s.checkPrologueGeneration(generator, expected)
...
func (s *GeneratorSuite) TestPrologueWithImportFromNestedInterface() {
	generator := s.getGenerator(
		"imports_from_nested_interface.go", "HasConflictingNestedImports",
	)
	expected := `// Package mocks contains autogenerated mocks.
package mocks

import fixtureshttp "github.com/shoenig/mockery3/v3/libmockery/fixtures/http"
import "net/http"
import "github.com/stretchr/testify/mock"
import test "github.com/shoenig/mockery3/v3/libmockery/fixtures"
